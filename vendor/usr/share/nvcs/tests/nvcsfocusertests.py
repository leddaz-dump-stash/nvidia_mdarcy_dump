...
class NvCSFocusPosTest(NvCSTestBase):
    ...
    def __init__(self, options, logger, sensorSetting):
        NvCSTestBase.__init__(self, options, logger, "FocusPos", sensorSetting)
    ...
    def runTest(self, args):
        ...
        step = (physicalRange[1] - physicalRange[0]) // (self.iterations - 1)
        ...
        for focusPos in focusPositions:
            ...
            if (self.nvrf.getFocusPosition() !=  focusPos):
                self.logger.error("FocusPosition value is not correct in the raw header: %d" % self.nvrf.getFocusPosition())
                retVal = NvCSTestResult.ERROR
                break
...
class NvCSSharpnessTest(NvCSTestBase):
    ...
    # This should select ROT_Width x ROT_Height image from the center of the image.
    ...
    def __init__(self, options, logger, sensorSetting):
        NvCSTestBase.__init__(self, options, logger, "Sharpness", sensorSetting)
        self.options = options
        self.sensorSetting = sensorSetting
    ...
    def _printSummary(self, focusPositions, sharpnessDictIteration1, sharpnessDictIteration2):
        ...
        sharpnessList = list(sharpnessDictIteration1.values())
        sharpnessList.extend(list(sharpnessDictIteration2.values()))

        maxSharpness = max(sharpnessList)

        for focusPosition in sharpnessDictIteration1.keys():
    ...
    def _getSharpnessAtFocusPosition(self, focusPosition):
        ...
        # this is for speedup
        retVal = nvcameraimageutils.cropRawImageFromCenter(self.nvrf, self.ROT_Width, self.ROT_Height)
        if(not retVal):
            self.logger.error("Unable to crop the image %s for crop width %d, height: %d " %
                              (rawFilePath, self.ROT_Width, self.ROT_Height))
